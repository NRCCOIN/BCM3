...
unsigned int nStakeMaxAge = 24 * 60 * 60 * 365 ;  // one year
...
const string strMessageMagic = "BitcoinMetal Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    printf("Block Number %d \n",nBestHeight);
        ...
        if(nBestHeight < YEARLY_BLOCKCOUNT)
            nRewardCoinYear = 10 * MAX_MINT_PROOF_OF_STAKE; //20% reward for 262800 blocks
        else if(nBestHeight < (2 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 9 * MAX_MINT_PROOF_OF_STAKE; //18% reward for 262800 * 2 blocks
        else if(nBestHeight < (3 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 8 * MAX_MINT_PROOF_OF_STAKE; //16% reward for 262800 * 3 blocks
        else if(nBestHeight < (4 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 7 * MAX_MINT_PROOF_OF_STAKE; //14% reward for 262800 * 4 blocks
        else if(nBestHeight < (5 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 6 * MAX_MINT_PROOF_OF_STAKE; //12% reward for 262800 * 5 blocks
        else if(nBestHeight < (6 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 5 * MAX_MINT_PROOF_OF_STAKE; //10% reward for 262800 * 5 blocks
        else if(nBestHeight < (7 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 4 * MAX_MINT_PROOF_OF_STAKE; //8% reward for 262800 * 5 blocks
        else if(nBestHeight < (8 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 3 * MAX_MINT_PROOF_OF_STAKE; //4% reward for 262800 * 5 blocks
        else if(nBestHeight < (9 * YEARLY_BLOCKCOUNT))
            nRewardCoinYear = 2 * MAX_MINT_PROOF_OF_STAKE; //6% reward for 262800 * 5 blocks
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitcoinMetal", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "This is a good start";
        CTransaction txNew;
        txNew.nTime = 1496234712;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x12ef339c4ba9c499d218d58d797cd847496f2aa3d30cca4c7aeb72a8ebfe8702"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0x12, 0xeb, 0x10 };
